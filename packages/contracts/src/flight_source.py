"""Flight source contract for FlySwarm (mirrors ``llm_provider.py``): a vendor-neutral
interface every agent/orchestrator calls instead of a provider SDK or raw HTTP.
Implementations live in ``packages/adapters/src/flights``. The canonical shapes
(``FlightOffer``, ``Money``, ``SearchQuery``) live in ``packages/domain`` and are
